...
func _ready() -> void:
	...
	if !player.is_authority:
		print("%s not authority" % NetworkTransport.is_server)
		return
	else:
		print("%s is authority" % NetworkTransport.is_server)
...
func _record_action_repeat_chat(message: String) -> void:
	...
	if !player.has_method("record_chat_event"):
		return
	player.record_chat_event(NetworkClient.username, message)
