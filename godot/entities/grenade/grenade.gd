...
func _ready() -> void:
	...
	if not _net.is_authoritative_instance:
		# Proxy: RigidBody3D-specific freeze. Collider disable in listen mode
		# is now framework-owned via NetPredictor._apply_listen_mode_role_gates;
		# the framework also hides the auth's visuals so we don't render both
		# grenades on the same physics body.
		freeze = true
